...
plan_generator = LlmAgent(
    ...
    **TOOL USE IS STRICTLY LIMITED:**
    ...
    ### [URL-RESTRICTION ADDED] ###
    **HARD URL RESTRICTION (MANDATORY):** If you use `google_search` for any reason, you MUST prefix every query with `site:google.github.io/adk-docs/`. Never search outside this URL and never remove this prefix.
    """,
    tools=[google_search],
)
...
section_researcher = LlmAgent(
    ...
    *   **Execution Directive:** You **MUST** systematically process every goal prefixed with `[RESEARCH]` before proceeding to Phase 2.
    ...
    ### [URL-RESTRICTION ADDED] ###
    **HARD URL RESTRICTION (MANDATORY):** All search queries MUST begin with the prefix `site:google.github.io/adk-docs/`. You may not issue, rewrite, or execute any query that would search outside this URL. If a query in the plan lacks this restriction, add the prefix before execution. If a query already has a different `site:` constraint, replace it with `site:google.github.io/adk-docs/`.

    *   For each `[RESEARCH]` goal:
        *   **Query Generation:** Formulate a comprehensive set of 4-5 targeted search queries. These queries must be expertly designed to broadly cover the specific intent of the `[RESEARCH]` goal from multiple angles. **Prefix every query with `site:google.github.io/adk-docs/`.**
        *   **Execution:** Utilize the `google_search` tool to execute **all** generated queries for the current `[RESEARCH]` goal. **Do not execute any query lacking the required `site:google.github.io/adk-docs/` prefix.**
...
enhanced_search_executor = LlmAgent(
    ...
    ### [URL-RESTRICTION ADDED] ###
    **HARD URL RESTRICTION (MANDATORY):** Before executing any follow-up query, ensure it is restricted by prefixing `site:google.github.io/adk-docs/`. If a follow-up query already has a different `site:` scope, replace it with `site:google.github.io/adk-docs/`. Never search outside this URL.
    """,
    tools=[google_search],
    output_key="section_research_findings",
    after_agent_callback=collect_research_sources_callback,
